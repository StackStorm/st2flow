...
export class ParameterEditor extends React.Component {
  ...
  render() {
    ...
    const positionField = {
      ...
      value: this.state.parameter.position,
      onChange: (value) => this.changeValue('position', value)
    };
...
export default class Meta extends React.Component {
  ...
  handleSubmit(event) {
    ...
    this.savePromise.resolve(this.state.meta);
    this.setState({ show: false, save: false });
  }
  ...
  handleParameterDelete(name) {
    ...
    this.changeValue('parameters', _.omitBy(parameters, (value, key) => name === key));
  }

  show(save) {
    this.componentWillReceiveProps(this.props);
    this.setState({ show: true, save });
  ...
  render() {
    ...
    const fields = [{
      ...
      children: [{
        ...
        options: _(this.state.packs).map('name').union(['default']).value()
      }, {
        name: 'dot',
        type: 'comment',
          ...
          <div {...contentProps} >
            ...
            <div className={ st2Class('status') }>
              <input type="submit"
                  form="metaform"
                  className={ st2Panel('field-button').and('field-button', 'positive') }
                  value={ this.state.save ? 'Save' : 'Update' } />
              <input type="button"
                  className={ st2Panel('field-button') }
                  onClick={ this.handleCancel.bind(this) }
                  value="Cancel" />
