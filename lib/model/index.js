...
export default class Model extends EventEmitter {
  ...
  fragments = {
    ...
    task: (params) => {
      ...
      const starter = specimen && specimen.key || defs.task //'  - '
          , indent = specimen && specimen.value && specimen.value[0].indent || defs.property // '    '
          ;
  ...
  parse(code) {
    ...
    state = _.transform(lines, this.definition.parseLine.bind(this.definition), state);
  ...
  _embedCoords() {
    ...
    const changes = _(this.tasks)
      ...
      .map(task => {
        ...
        const sector = task.getSector('coord')
            , value = this.fragments.coord(task, Math.round(x), Math.round(y))
            ;
    ...
    this.virtualEditor.bulkReplace(changes);
  }
}
